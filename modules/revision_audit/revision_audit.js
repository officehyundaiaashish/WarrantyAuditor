 ...
 * Self-contained module: Injects CSS, HTML modal, and workspace version controls.
 */
...
(function () {
    ...
    async function confirmCreateRevision() {
        ...
        try {
            ...
            family.forEach(item => {
                if (!window.dbIndex[item.key].revision_number) {
                    window.dbIndex[item.key].revision_number = item.revNum;
                }
                if (!window.dbIndex[item.key].root_key) {
                    window.dbIndex[item.key].root_key = rootKey;
                }
            });
